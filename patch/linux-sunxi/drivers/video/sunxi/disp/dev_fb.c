...
static char *screen0_output_mode="640x480p60"; /* set to 640x480 for vga console */
...
static u32 tv_mode_to_frame_rate(u32 mode)
{
	...
	case DISP_TV_MOD_1080P_50HZ:
		return 50;
	case DISP_TV_MOD_H640_V480_72HZ:
		return 72;
	default:
		return 60;
	}
...
#ifdef CONFIG_FB_SUNXI_RESERVED_MEM
	info->fix.smem_len = PAGE_ALIGN( 1920 * 4 * 1080 * fb_para->buffer_num);
#else
...
#ifdef CONFIG_FB_SUNXI_RESERVED_MEM
			g_fbi.fbinfo[i]->fix.smem_len = PAGE_ALIGN(1920*4*1080*2);
#else
...
#ifdef CONFIG_SUNXI_DVI_FIX
			fb_para.output_width = ( fb_para.output_width * 98 )/100;
			fb_para.output_height = ( fb_para.output_height * 98 )/100;
#endif
