...
#ifdef CONFIG_HARDWARE_LIB_I2C
#include <linux/miscdevice.h>
#include <linux/uaccess.h>
#include <linux/fs.h>
#define I2CCLOCK_CHANGE 0x0740
static struct sunxi_i2c *i2c2_handle = NULL;
#endif
...
#ifdef CONFIG_HARDWARE_LIB_I2C
static int i2c_sunxi_clk_change(struct sunxi_i2c *i2c, unsigned int bus_freq)
{
    int ret = 0;
    ...
    i2c->bus_freq = bus_freq;
    i2c_dbg("twi%d, apb clock = %d, bus clock = %d \n",i2c->bus_num, apb_clk, i2c->bus_freq);
    aw_twi_set_clock(apb_clk, i2c->bus_freq, i2c->base_addr);

    return 0;
}

static long hwi2c_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    int ret = 0;
    ...
    switch (cmd) {
    case I2CCLOCK_CHANGE:
         i2c_dbg("%s I2CCLOCK_CHANGE \n", __func__);
         if (get_user(speed_hz, (unsigned int __user *)arg)) {
            ret = -EFAULT;
            break;
         }

         if ((speed_hz <= 400000) && (speed_hz > 0) && i2c2_handle) {
            i2c_sunxi_clk_change(i2c2_handle, speed_hz);
         }else{
            ret = -EFAULT;
         }
         break;
...
#ifdef CONFIG_HARDWARE_LIB_I2C
static const struct file_operations hwi2c_fops = {
    .owner      = THIS_MODULE,
    .unlocked_ioctl = hwi2c_ioctl,
    .llseek     = no_llseek,
};
